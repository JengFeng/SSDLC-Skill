# 階段 02：系統設計 — Skill 配置
本檔案由 AI 代理自動生成與維護。
## 標準產出清單（七項，Mermaid 格式優先）
| 產出 | 負責 Skill | 格式 |
|:---|:---|:---|
| db_schema.sql | 13-prisma | SQL DDL |
| er_diagram.md | 10-mermaid | Mermaid erDiagram |
| api_spec.md | 11-openapi_generator | Markdown |
| ui_prototype.html | 03-bootstrap-ui | HTML (Bootstrap) |
| use_case_diagram.md | 12-plantuml / 10-mermaid | Mermaid graph |
| activity_diagram.md | 12-plantuml / 10-mermaid | Mermaid flowchart |
| sequence_diagram.md | 12-plantuml / 10-mermaid | Mermaid sequenceDiagram |
> 註：.puml 原始檔可保留供 PlantUML 工具使用，.md 為瀏覽器可直接渲染格式。
