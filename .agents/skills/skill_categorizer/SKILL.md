---
name: skill-categorizer
description: 當使用者要求「歸類 skill」、「技能歸類」、「匯入外部 skill」或「分類技能」時觸發。此技能定義了如何將外部下載的 Anthropic 官方技能或 Benson 技能依據 SSDLC 6 個開發階段進行自動分類、檔案複製、來源加註及更新 README.md 索引表的 SOP。
---

# 技能歸類技能 (Skill Categorizer)

本技能定義了將外部下載的技能檔案（例如來自 `anthropics/skills` 或 `Benson-skill`）自動歸類至軟體生命週期（SSDLC）各階段的標準作業流程。
