...
> 生成日期：2026-06-29 | 階段：Phase 01 規劃與需求分析
...
| 編號 | 事項 | 優先級 | 負責方 |
|:---|:---|:---|:---|
| OI-001 | AD 網域控制站版本與 LDAP 連線參數（Base DN、Bind Account） | P0 | IT 基礎設施 |
| OI-002 | 附件儲存方式：資料庫 BLOB vs 檔案伺服器 vs NAS 路徑 | P0 | IT 基礎設施 |
| OI-003 | 薪資欄位是否需要與現有薪資系統介接？目前訪談僅提及欄位權限 | P1 | HR |
| OI-004 | 「高階主管」的界定標準（副總以上？處長以上？） | P0 | HRM |
| OI-005 | 目前員工總數與預估年成長率（用於容量規劃） | P1 | HRM |
| OI-006 | 排班系統是否需要與本系統介接？（訪談提及排班在別的系統） | P2 | HR |
