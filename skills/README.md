...
### Anthropic 官方技能 (6)
* **[doc-coauthoring](skills/01_planning_and_analysis/doc-coauthoring)**：文件協同撰寫引導。提供結構化工作流以協同撰寫提案、規格書或決策文件。
...
* **[internal-comms](skills/01_planning_and_analysis/internal-comms)**：內部溝通撰寫資源。提供狀態報告、事件報告及專案更新等內部通訊文件格式。
...
* **[docx](skills/01_planning_and_analysis/docx)**：Word 文件處理。讀取、編輯、操作與排版 `.docx` 檔案。 🌐 通用
...
* **[xlsx](skills/01_planning_and_analysis/xlsx)**：Excel 試算表處理。進行資料分析、公式計算、格式化與表格清洗。 🌐 通用
...
* **[pdf](skills/01_planning_and_analysis/pdf)**：PDF 處理與 OCR。支援 PDF 檔案的拆合、表單填寫與 OCR 文字提取。 🌐 通用
...
### GitHub 推薦開源工具技能 (4)
* **[[01] langchain](skills/01_planning_and_analysis/langchain)**：需求場景拆解、語意整理與鏈式呼叫工具。
...
* **[[02] llamaindex](skills/01_planning_and_analysis/llamaindex)**：文件數據索引萃取與關鍵需求擷取工具。
...
* **[[03] docling](skills/01_planning_and_analysis/docling)**：各式複雜文件（PDF, Word等）轉換為標準 Markdown 格式工具。
...
* **[[04] docusaurus](skills/01_planning_and_analysis/docusaurus)**：需求文件靜態版本網站建置與版本化管理。
...
### GitHub 推薦開源文檔技能合集 (1)
* **[[19] document-skills](skills/01_planning_and_analysis/document-skills)**：增強版文件處理技能合集，涵蓋 docx、pdf、pptx、xlsx 四種格式的進階操作、OOXML 底層編輯、HTML 轉換與試算表重算。
...
### Anthropic 官方技能 (6)
* **[frontend-design](skills/02_system_design/frontend-design)**：前端 UI 視覺設計引導。協助規劃字型排版與視覺方向，避免模板化設計。
...
* **[theme-factory](skills/02_system_design/theme-factory)**：配色主題工具。提供 10 種預設配色與字型主題，可套用至簡報、網頁或文件。
...
* **[brand-guidelines](skills/02_system_design/brand-guidelines)**：品牌色彩規範。套用 Anthropic 官方配色與字型等設計標準。
...
* **[canvas-design](skills/02_system_design/canvas-design)**：靜態視覺與海報設計。用於建立高品質的 PNG 與 PDF 靜態藝術海報。
...
* **[algorithmic-art](skills/02_system_design/algorithmic-art)**：演算法藝術生成。使用 p5.js 與隨機參數生成互動式視覺藝術。
...
* **[[05] brand](skills/02_system_design/brand)**：品牌識別指南 — 品牌聲音 / 視覺識別 / 訊息框架 / 資產管理 / 一致性檢查。內含 11 份參考文件與品牌模板。
...
* **[[06] design](skills/02_system_design/design)**：通用設計工具包 — Logo 設計（55 風格）/ CIP 企業識別（50 交付物）/ 圖示設計 / 社群素材。
...
* **[[07] design-system](skills/02_system_design/design-system)**：Design Token 架構 — 三層 token（primitive→semantic→component）/ CSS 變數 / 元件規格 / Tailwind 整合。
...
* **[[08] ui-styling](skills/02_system_design/ui-styling)**：前端樣式參考 — shadcn/ui 元件 / Tailwind CSS / 響應式佈局 / 暗色模式 / 無障礙組件。 🌐 雙歸屬（Phase 02 設計規範 + Phase 03 實作落地）
...
* **[[09] banner-design](skills/02_system_design/banner-design)**：多格式橫幅設計 — 22 風格 / 社群+廣告+網頁+印刷 / AI 生成視覺素材。 ⚠️ 依賴 ui-ux-pro-max
...
### Anthropic 官方技能 (4)
* **[web-artifacts-builder](skills/03_implementation_and_coding/web-artifacts-builder)**：網頁應用建置。使用 React、Tailwind CSS 與 shadcn/ui 設計多組件的前端應用。
...
* **[mcp-builder](skills/03_implementation_and_coding/mcp-builder)**：MCP 伺服器建置。使用 Python 或 Node.js/TypeScript 開發 Model Context Protocol 伺服器以對接外部服務。
...
* **[claude-api](skills/03_implementation_and_coding/claude-api)**：Claude API 整合指引。提供 API 參數、計價、Tokens 計算與快取的參考手冊。
...
### GitHub 推薦開源工具技能 (5)
* **[[01] continue_dev](skills/03_implementation_and_coding/continue_dev)**：本機 AI 輔助寫碼、語境理解與程式產生套件。
...
* **[[02] codellama](skills/03_implementation_and_coding/codellama)**：本地離線代碼編譯、語法生成與程式碼自動補全核心。
...
* **[[03] eslint](skills/03_implementation_and_coding/eslint)**：代碼語法與靜態邏輯檢核，維護團隊代碼品質。
...
* **[[04] prettier](skills/03_implementation_and_coding/prettier)**：代碼風格美化與自動格式化，避免排版衝突。
...
* **[[05] nx](skills/03_implementation_and_coding/nx)**：大型專案多模組與依賴關係的整合管控工具。
...
* **[[05] pytest](skills/04_testing/pytest)**：Python 單元測試與多功能測試驗證框架。
  * **追溯來源**：[本機外部資源目錄](../external-resources/github-skills/04_testing/pytest) / [原始 GitHub 倉庫](https://github.com/pytest-dev/pytest)
* **[[06] jest](skills/04_testing/jest)**：JavaScript / TypeScript 的單元測試與 Mock 驗證工具。
  * **追溯來源**：[本機外部資源目錄](../external-resources/github-skills/04_testing/jest) / [原始 GitHub 倉庫](https://github.com/jestjs/jest)
* **[[07] sonarqube](skills/04_testing/sonarqube)**：專案原始碼安全漏洞、壞味道與代碼品質靜態掃描。
  * **追溯來源**：[本機外部資源目錄](../external-resources/github-skills/04_testing/sonarqube) / [原始 GitHub 倉庫](https://github.com/SonarSource/sonarqube)
* **[[08] coverage_py](skills/04_testing/coverage_py)**：Python 測試覆蓋率分析與未涵蓋代碼報告器。
* **[[09] systematic-debugging](skills/04_testing/systematic-debugging)**：系統化除錯方法。在遇到任何 bug、測試失敗或非預期行為時，於提出修復方案之前先進行根因分析。
...
### GitHub 推薦開源工具技能 (3)
* **[[01] docker](skills/05_deployment/docker)**：容器化服務打包、鏡像製作與多服務 Docker Compose 部署。
  * **追溯來源**：[本機外部資源目錄](../external-resources/github-skills/05_deployment/docker) / [原始 GitHub 倉庫](https://github.com/docker/docker-ce)
* **[[02] ansible](skills/05_deployment/ansible)**：伺服器自動化組態管理、主機配置與遠端批次部署。
  * **追溯來源**：[本機外部資源目錄](../external-resources/github-skills/05_deployment/ansible) / [原始 GitHub 倉庫](https://github.com/ansible/ansible)
* **[[03] nginx_config_generator](skills/05_deployment/nginx_config_generator)**：Web 伺服器反向代理與負載平衡組態檔自動產生器。
...
* **[[02] prometheus_grafana](skills/06_maintenance/prometheus_grafana)**：系統硬體指標與服務效能監控、即時 Grafana 圖表告警機制。
...
* **[[03] opentelemetry](skills/06_maintenance/opentelemetry)**：雲原生 APM 效能瓶頸、調用鏈分佈式追蹤系統。
...
* **[[04] logparser](skills/06_maintenance/logparser)**：雜亂日誌自動清理、正則篩選與格式化輸出分析。
...
* **[[12] docx](skills/00_cross_phase/docx)**：Word 文件處理。讀取、編輯、操作與排版 `.docx` 檔案。🌐 通用
  * **追溯來源**：[本機外部資源目錄](../external-resources/anthropics-skills/skills/docx) / [原始 GitHub 倉庫](https://github.com/anthropics/skills/tree/main/skills/docx)

* **[[13] xlsx](skills/00_cross_phase/xlsx)**：Excel 試算表處理。進行資料分析、公式計算、格式化與表格清洗。🌐 通用
  * **追溯來源**：[本機外部資源目錄](../external-resources/anthropics-skills/skills/xlsx) / [原始 GitHub 倉庫](https://github.com/anthropics/skills/tree/main/skills/xlsx)

* **[[14] pdf](skills/00_cross_phase/pdf)**：PDF 處理與 OCR。支援 PDF 檔案的拆合、表單填寫與 OCR 文字提取。🌐 通用
  * **追溯來源**：[本機外部資源目錄](../external-resources/anthropics-skills/skills/pdf) / [原始 GitHub 倉庫](https://github.com/anthropics/skills/tree/main/skills/pdf)
...
* **[[01] langgraph](skills/00_cross_phase/langgraph)**：複雜狀態多 Agent 協作工作流的圖形狀態管理引擎。
...
* **[[02] git](skills/00_cross_phase/git)**：全域版本控制、分支管理與 Baseline 基線封存追溯工具。
  * **追溯來源**：[本機外部資源目錄](../external-resources/github-skills/00_cross_phase/git) / [原始 GitHub 倉庫](https://github.com/git/git)
* **[[03] diffsync](skills/00_cross_phase/diffsync)**：跨階段專案原始碼版本差異對比與同步工具。
* **[[04] autoresearch](skills/00_cross_phase/autoresearch)**：自主目標導向迭代循環。自動修改、驗證、保留/丟棄，針對任意指標進行最佳化，靈感來自 Karpathy。
...
* **[[05] brainstorming](skills/00_cross_phase/brainstorming)**：創意發想引導。在任何創意工作前探索使用者意圖、需求與設計方向。
...
* **[[06] firecrawl](skills/00_cross_phase/firecrawl)**：網頁內容擷取、截圖、結構化資料提取、網頁搜尋與文件網站爬蟲。
...
* **[[07] test-driven-development](skills/00_cross_phase/test-driven-development)**：測試驅動開發流程。在撰寫實作程式碼之前先寫測試。
...
* **[[08] verification-before-completion](skills/00_cross_phase/verification-before-completion)**：完成前驗證。在聲稱工作完成、修復或通過之前執行驗證命令並確認輸出。
...
* **[[09] writing-plans](skills/00_cross_phase/writing-plans)**：撰寫實作計畫。在接觸程式碼之前，先制定多步驟任務的規格與需求。
...
* **[[10] ralph-loop](skills/00_cross_phase/ralph-loop)**：自主 AI 開發循環。自動化修改→測試→驗證→保留/丟棄的迭代流程，具備智慧退出偵測。
...
* **[[11] using-superpowers](skills/00_cross_phase/using-superpowers)**：Skill 尋找與使用引導。教導 AI 代理如何在對話中主動發現、呼叫與使用可用技能。
...
* **[[13] ui-ux-pro-max](skills/00_cross_phase/ui-ux-pro-max)**：設計智慧搜尋引擎 — Python 搜尋引擎 + 14 個 CSV 資料庫（84 風格 / 161 色板 / 73 字型配對 / 99 UX 指南 / 25 圖表類型 / 17 技術棧）。支援 `product`/`style`/`typography`/`color`/`landing`/`chart`/`ux` 七大領域即時查詢。 ⚠️ 為其他 UI/UX 技能的基礎依賴。
...
* **[[18] slides](skills/00_cross_phase/slides)**：HTML 簡報製作 — Chart.js 資料視覺化 / 文案公式 / 版型策略 / 響應式簡報。所有階段皆可用於製作專案簡報。
