...
在使用任何指令之前，請先執行以下步驟，讓 AI 代理載入本框架的規章與規則：

**Step 1 — 告誟 AI 代理讀取框架規章**
在你的 AI 開發工具（Cursor / Windsurf / VS Code + Copilot / Codex 等）的對話中輸入：
```
請先讀取 AGENTS.md 以及 .agents/AGENTS.md 與 docs/CORE_RULES.md
```
AI 代理讀取後，框架的指令協定、Skill 管理規則、階段關卡機制等才會生效。

> **⚠️ 為什麼需要這一步？**
> 不同的 AI 開發工具預設不會自動載入專案的規章檔案。如果不先讀取，後繼的 @init、@01 等指令將無法被正確解析，AI 代理會把它們當作普通文字對待。
>
> **一次性設定**：首次讀取後，同一個對話 session 中不需要重複執行。開啟新 session 時需要重新讀取。

**Step 2 — 確認框架已載入**
AI 代理回覆確認已讀取後，可輸入以下指令驗證：
```
@stages
```
若正確顯示六大開發階段代碼，表示框架已就緒。
...
```
> ⚠️ **此為選用功能**。`@init` 時 AI 代理會詢問是否啟用。啟用後只需執行 `@io set [階段]` 定義該階段的輸入需求與產出承諾即可。**不啟用也不影響開發流程**，新手可直接跳過。後續隨時可透過 `@io set` 啟用。
> 
