...
### 4. （選用）定義階段輸入與輸出檔案
```
@io list              # 列出各階段預設 IO 速查表
@io set 02            # 互動式設定 Phase 02 的輸入輸出
@io 02                # 跨階段勾稽檢查
@io diff 02 03        # 比對 Phase 02 與 03 的 IO 差異
```
> ⚠️ **此為選用功能**。`@init` 時 AI 代理會詢問是否啟用。啟用後每個階段的輸入需求與產出承諾會被顯式定義於 `io_files.yaml`，供後續跨階段自動勾稽。**不啟用也不影響開發流程**，新手可直接跳過。後續隨時可透過 `@io set` 啟用。
