...
- 說出「**建立基線**」、「**新建 Baseline**」、「**儲存專案快照**」→ AI 代理自動執行 @baseline，建立可獨立執行的完整專案快照。
...
| **`@baseline`** | 無 | 建立可獨立執行的完整專案快照至 `baseline/` 目錄（含原始碼、模板、資料庫、部署腳本），自動遞增版本號，保留最近 3 份，舊版自動清理。口語觸發：「建立基線」、「新建 Baseline」。 | `@baseline` |
