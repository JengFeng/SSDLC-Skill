...
| **`@restore`** | `latest` / `N`（1~5） / `YYYYMMDD-HHMMSS` | 回溯工作目錄至指定執行快照。自動 `git stash` 保留未提交變更 → `git apply` 載入差異補丁 → SHA-256 驗證還原完整性。失敗時輸出衝突清單，不強制覆蓋。口語觸發：「回溯快照」「還原快照」「退回上一步」。 | `@restore latest` |
...
*   **2026-06-27 (@unlock 使用說明補強)**：
    ...
    *   新增「## 四、 @unlock 指令使用時機與方式詳解」章節，含三種合法情境、執行步驟、注意事項。
